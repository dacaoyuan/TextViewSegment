 ...
 * @author YuanPeikai
 * @Package
 * @Description:自定义 TextView 分段
 * @date 2019/12/20 14:59
 * <p>
 * todo 后期优化，这里自定义view可封装成builder模式
 */
...
public class SegmentTextView extends AppCompatTextView {
    ...
    public void setSpecifiedTextsColor(String text, String specifiedTexts, int color, boolean is_underline, StringClickSpan.ClickCallBack clickCallBack) {
        ...
        for (Integer i : sTextsStartList) {
            if (is_underline) {
                styledText.setSpan(new StringClickSpan(context, color, clickCallBack), i, i + sTextLength, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            } else {
                //System.out.println("SegmentTextView.setSpecifiedTextsColor i=" + i + " i + sTextLength=" + (i + sTextLength));
                styledText.setSpan(new StringClickSpan(context, color, false, clickCallBack), i, i + sTextLength, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        setText(styledText);
